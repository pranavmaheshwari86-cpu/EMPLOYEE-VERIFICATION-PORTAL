...
import { Building2, Mail, MapPin, Phone, Shield, Link, Globe } from 'lucide-react';
    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div>
          ...
          <p className="text-slate-400 mt-2">Manage your company information and settings.</p>
        </div>
            ...
            <div className="relative flex justify-between items-end -mt-12 mb-8">
              <div className="flex items-end space-x-5">
                <div className="w-24 h-24 bg-[#1a1f2e] border-4 border-[#131722] rounded-2xl flex items-center justify-center relative overflow-hidden z-10 shadow-xl shadow-black/50">
                  {user?.profileImage ? (
                    <img src={user.profileImage} alt={user?.companyName || user?.name} className="w-full h-full object-cover" />
                  ) : (
                    <Building2 className="w-10 h-10 text-slate-400" />
                  )}
                </div>
                <div className="pb-2">
                  <h2 className="text-2xl font-bold text-white">{user?.companyName || user?.name}</h2>
                  <p className="text-primary font-medium">Employer Account</p>
                </div>
              </div>
                    ...
                    {isEditing ? (
                      <div className="space-y-4 max-w-md">
                        ...
                        <div>
                          ...
                          <input type="text" name="companyName" value={formData.companyName} onChange={handleChange} className="w-full bg-[#1a1f2e] border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-primary transition-colors" />
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <input type="text" name="github" value={formData.github} onChange={handleChange} placeholder="e.g. https://github.com/company" className="w-full bg-[#1a1f2e] border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-primary transition-colors" />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-slate-400 mb-1">LinkedIn Profile</label>
                          <input type="text" name="linkedin" value={formData.linkedin} onChange={handleChange} placeholder="e.g. https://linkedin.com/company/name" className="w-full bg-[#1a1f2e] border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-primary transition-colors" />
                        </div>
                      </div>
                    ) : (
                      <>
